 ...
 * @beta
 */
function applyDefinitions(table, definitions) {
    ...
    for (const { name, fn } of definitions) {
        ...
        const buildAt = (idx) => {
            const r = {};
            for (const [c, vals] of Object.entries(columns))
                r[c] = vals[idx];
            return r;
        };
...
function resolveRowCount(table) {
    ...
    for (const len of lengths) {
        if (len !== first) {
            throw new Error(`Table columns have unequal lengths: expected ${String(first)}, found ${String(len)}.`);
        }
    }
    return first;
}

export { BoundEngine, ChainedBoundEngine, Engine, applyDefinitions, def };
//# sourceMappingURL=nosheet.esm.js.map
